...
static int count = 10000; // Default size of the tree
#define SAMPLE 100
...
static timing_info_t timing_info, timing_info_map;
...
static std::vector<unsigned long> key_vector;
static std::vector<unsigned long> ne_key_vector;

struct root {
	pmem::obj::persistent_ptr<kv_type> kv;
};

static std::map<unsigned long, std::shared_ptr<value_type>> mymap;
...
insert_elements_kv(pmem::obj::pool<root> pop)
{
	...
	for (int i = 0; i < count; i++) {
		...
		timing_info_map.diff +=
			(timing_info_map.end - timing_info_map.start) >> 1;
	}
...
lookup_elements_kv(pmem::obj::pool<root> pop)
{
	...
	for (auto &key : key_vector) {
		...
		// assert(it != r->kv->end());
		// std::cerr << "key: " << key << " index: " <<
		// it->value()->index << std::endl;
	...
	timing_info.diff = (timing_info.end - timing_info.start) >> 1;
	timing_info.diff = timing_info.diff / key_vector.size();
	std::cerr << "Average access time (map): " << timing_info.diff
		  << std::endl;
}
...
main(int argc, char *argv[])
{
	...
	try {
		...
		std::cerr << "Looking up " << key_vector.size()
			  << " elements..." << std::endl;
		lookup_elements_kv(pop);
		std::cerr << "[Key mot present] Looking up "
			  << ne_key_vector.size() << " elements..."
			  << std::endl;
